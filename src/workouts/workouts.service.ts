...
import { CreateWorkoutPlanDto, UpdateWorkoutPlanDto, AssignWorkoutDto, UpdateWorkoutAssignmentDto } from './dto/workout.dto';
...
export class WorkoutsService {
  ...
  async findAll(gymId: number, filters?: {
    ...
    const { workouts, total } = await this.tenantService.executeInTenant(gymId, async (client) => {
      let whereClause = '1=1';
      const values: any[] = [];
      let paramIndex = 1;
   ...
   */
  async create(dto: CreateWorkoutPlanDto, gymId: number, userId: number, branchId?: number | null) {
    await this.ensureTablesExist(gymId);
  ...
  async update(id: number, dto: UpdateWorkoutPlanDto, gymId: number) {
    ...
    if (dto.difficulty) { updates.push(`difficulty = $${paramIndex++}`); values.push(dto.difficulty); }
    if (dto.duration !== undefined) { updates.push(`duration = $${paramIndex++}`); values.push(dto.duration); }
    if (dto.sessionsPerWeek !== undefined) { updates.push(`sessions_per_week = $${paramIndex++}`); values.push(dto.sessionsPerWeek); }
    if (dto.estimatedSessionDuration !== undefined) { updates.push(`estimated_session_duration = $${paramIndex++}`); values.push(dto.estimatedSessionDuration); }
    if (dto.exercises !== undefined) { updates.push(`exercises = $${paramIndex++}`); values.push(JSON.stringify(dto.exercises)); }
   ...
   */
  async assignWorkout(dto: AssignWorkoutDto, gymId: number, assignedBy: number, branchId?: number | null) {
    await this.ensureTablesExist(gymId);
    ...
    if (existingAssignment) {
      throw new ConflictException('User already has this workout plan assigned');
    }

    // Get user and workout details for the response
    const { assignment, workout, user, assignedByUser } = await this.tenantService.executeInTenant(gymId, async (client) => {
      ...
      const assignmentResult = await client.query(
        `INSERT INTO workout_assignments (branch_id, workout_plan_id, user_id, assigned_by, assigned_at, status, progress_percentage, notes, created_at, updated_at)
         ...
         RETURNING *`,
        [branchId ?? null, dto.workoutPlanId, dto.userId, assignedBy, dto.notes || null]
      );

      // Get workout details
      const workoutResult = await client.query(`SELECT * FROM workout_plans WHERE id = $1`, [dto.workoutPlanId]);
    ...
    return this.formatWorkoutAssignment(assignment, workout, user, assignedByUser);
  }
   ...
   */
  async updateAssignment(assignmentId: number, dto: UpdateWorkoutAssignmentDto, gymId: number) {
    await this.ensureTablesExist(gymId);
    ...
    if (!assignment) {
      throw new NotFoundException(`Workout assignment with ID ${assignmentId} not found`);
    }
    ...
    if (dto.status) { updates.push(`status = $${paramIndex++}`); values.push(dto.status); }
    if (dto.progressPercentage !== undefined) { updates.push(`progress_percentage = $${paramIndex++}`); values.push(dto.progressPercentage); }
  ...
  private formatWorkoutAssignment(assignment: any, workout: any, user: any, assignedByUser: any) {
    return {
      id: assignment.id,
