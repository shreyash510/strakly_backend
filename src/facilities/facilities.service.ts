...
export class FacilitiesService {
  ...
  async findOne(id: number, gymId: number, branchId: number | null = null) {
    const facility = await this.tenantService.executeInTenant(gymId, async (client) => {
   ...
   */
  async create(dto: CreateFacilityDto, gymId: number, branchId: number | null = null) {
    ...
    if (existing) {
      throw new ConflictException(`Facility with code ${dto.code} already exists`);
    }
