...
export class DietsService {
  ...
  async ensureTablesExist(gymId: number): Promise<void> {
    await this.tenantService.executeInTenant(
      ...
      `);
        ...
        // Create indexes
        await client.query(
          `CREATE INDEX IF NOT EXISTS "idx_${schemaName}_diets_status" ON "${schemaName}"."diets"(status)`,
        );
        await client.query(
          `CREATE INDEX IF NOT EXISTS "idx_${schemaName}_diets_type" ON "${schemaName}"."diets"(type)`,
        );
        await client.query(
          `CREATE INDEX IF NOT EXISTS "idx_${schemaName}_diets_category" ON "${schemaName}"."diets"(category)`,
        );
        await client.query(
          `CREATE INDEX IF NOT EXISTS "idx_${schemaName}_diet_assignments_diet" ON "${schemaName}"."diet_assignments"(diet_id)`,
        );
        await client.query(
          `CREATE INDEX IF NOT EXISTS "idx_${schemaName}_diet_assignments_user" ON "${schemaName}"."diet_assignments"(user_id)`,
        );
        await client.query(
          `CREATE INDEX IF NOT EXISTS "idx_${schemaName}_diet_assignments_status" ON "${schemaName}"."diet_assignments"(status)`,
        );
      },
    );
  }
  ...
  async findAll(
    ...
    const { diets, total } = await this.tenantService.executeInTenant(
      gymId,
      async (client) => {
        let whereClause = '1=1';
  ...
  async assignDiet(
    ...
    const { assignment, diet, user, assignedByUser } =
      await this.tenantService.executeInTenant(gymId, async (client) => {
  ...
  async updateAssignment(
    ...
    if (!assignment) {
      throw new NotFoundException(
        `Diet assignment with ID ${assignmentId} not found`,
      );
    }
