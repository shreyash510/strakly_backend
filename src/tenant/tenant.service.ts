...
export class TenantService implements OnModuleDestroy {
  ...
  /**
   * Create the base expense tables (expense_categories + expenses) and seed the
   * default system categories. Mirrors migrations/tenant/003_tenant_add_expenses.sql.
   * Idempotent (CREATE TABLE IF NOT EXISTS / ON CONFLICT DO NOTHING).
   *
   * Must run before addBranchIdColumns and addExpenseControlColumns, both of which
   * ALTER the expenses table.
   */
  private async createExpensesTables(client: PoolClient, schemaName: string): Promise<void> {
    try {
      await client.query(`
      ...
      await client.query(`
        CREATE TABLE IF NOT EXISTS "${schemaName}"."expenses" (
          id SERIAL PRIMARY KEY,
          category_id INTEGER REFERENCES "${schemaName}"."expense_categories"(id),
      ...
      for (const stmt of [
        `CREATE INDEX IF NOT EXISTS "idx_${schemaName}_expense_categories_code" ON "${schemaName}"."expense_categories"(code)`,
        `CREATE INDEX IF NOT EXISTS "idx_${schemaName}_expense_categories_is_active" ON "${schemaName}"."expense_categories"(is_active)`,
        `CREATE INDEX IF NOT EXISTS "idx_${schemaName}_expenses_category_id" ON "${schemaName}"."expenses"(category_id)`,
        `CREATE INDEX IF NOT EXISTS "idx_${schemaName}_expenses_expense_date" ON "${schemaName}"."expenses"(expense_date)`,
        `CREATE INDEX IF NOT EXISTS "idx_${schemaName}_expenses_payment_status" ON "${schemaName}"."expenses"(payment_status)`,
        `CREATE INDEX IF NOT EXISTS "idx_${schemaName}_expenses_is_recurring" ON "${schemaName}"."expenses"(is_recurring)`,
        `CREATE INDEX IF NOT EXISTS "idx_${schemaName}_expenses_is_deleted" ON "${schemaName}"."expenses"(is_deleted) WHERE is_deleted = FALSE`,
        `CREATE INDEX IF NOT EXISTS "idx_${schemaName}_expenses_paid_by_id" ON "${schemaName}"."expenses"(paid_by_id)`,
      ]) {
        await client.query(stmt);
      }

      await client.query(`
        INSERT INTO "${schemaName}"."expense_categories" (name, code, icon, color, sort_order, is_system) VALUES
      ...
      this.logger.error(
        `Failed to create expense tables in ${schemaName}:`,
        error instanceof Error ? error.message : String(error),
      );
    }
  }
  ...
  private async addBranchIdColumns(client: PoolClient, schemaName: string): Promise<void> {
    const tables = [
      'users', 'leads', 'referrals', 'plans', 'offers', 'memberships', 'products',
